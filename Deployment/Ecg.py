...
class ECG:
	...
	def GrayImgae(self,image):
		...
		if image.shape[-1] == 4:  # RGBA image
				...
				return image.squeeze()  # Remove the channel axis
		elif image.shape[-1] != 3:  # Not RGB, RGBA, or grayscale
				raise ValueError(f"Expected an RGB, RGBA, or grayscale image, got shape {image.shape}")
			
			# Now the image is guaranteed to be RGB (3 channels)
